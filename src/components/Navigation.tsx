...
const navItems = [
  { href: '/', label: 'ホーム', icon: '⌂' },
  { href: '/diagnostic', label: 'テスト', icon: '◉' },
  { href: '/courses', label: 'コース', icon: '◈' },
  { href: '/reference', label: '早見表', icon: '⌨' },
...
export default function Navigation() {
  ...
  return (
    <nav style={{ background: 'rgba(7,9,14,0.97)', borderBottom: '1px solid #131e2e', backdropFilter: 'blur(8px)', position: 'sticky', top: 0, zIndex: 100 }}>
      <div className="nav-accent-line" />
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 52 }}>
          {/* Logo */}
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, background: 'linear-gradient(135deg, #2a1f5e, #1a1240)', border: '1px solid var(--accent)', color: 'var(--accent)' }}>
              ◆
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            {navItems.map(item => {
              ...
              const isCmd = item.href === '/reference';
              return (
                <Link key={item.href} href={item.href} style={{
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
                  padding: isCmd ? '5px 10px' : '6px 10px',
                  borderRadius: isCmd ? 6 : 8,
                  fontWeight: 600, letterSpacing: '0.04em',
                  textDecoration: 'none', minWidth: 40, transition: 'all 0.15s',
                  background: active ? 'rgba(157,140,255,0.12)' : 'transparent',
                  color: active ? 'var(--accent)' : 'var(--text-secondary)',
                  border: active
                    ? '1px solid rgba(157,140,255,0.3)'
                    : isCmd
                      ? '1px solid rgba(157,140,255,0.12)'
                      : '1px solid transparent',
                  fontFamily: isCmd ? 'var(--font-mono)' : 'inherit',
                }}>
                  <span style={{ fontSize: isCmd ? 12 : 14, lineHeight: 1 }}>{item.icon}</span>
            ...
            <button onClick={handleShare} style={{
              ...
              background: shared ? 'rgba(45,206,137,0.1)' : 'transparent',
