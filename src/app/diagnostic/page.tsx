    ...
    <>
      ...
      <main style={{ maxWidth: 680, margin: '0 auto', padding: '48px 16px 80px' }} className="dot-bg">
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 72, height: 72, borderRadius: 16, background: 'rgba(157,140,255,0.08)', border: '1px solid rgba(157,140,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, margin: '0 auto 24px', color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>◉</div>
          <h1 style={{ fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: 700, margin: '0 0 12px', letterSpacing: '-0.01em' }}>レベルテスト</h1>
          <p style={{ color: 'var(--text-secondary)', lineHeight: 1.8, maxWidth: 480, margin: '0 auto 36px', fontSize: 15 }}>
            {questions.length}問の問題から、現在のレベルと弱点パートを判定します。
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 40 }}>
            {[
              ...
              { label: '所要時間', value: '15〜20分' },
              { label: '対象', value: '全6パート' },
            ].map(s => (
    ...
    <>
      ...
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '40px 16px 80px' }}>
        ...
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '28px 24px', marginBottom: 20, textAlign: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: 12, background: 'rgba(45,206,137,0.1)', border: '1px solid rgba(45,206,137,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: 'var(--green)', margin: '0 auto 16px' }}>✓</div>
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: '0 0 8px' }}>テスト完了</h1>
              ...
              {result.recommendedCourseIds.slice(0, 6).map(id => {
                ...
                return (
                  <Link key={id} href={`/courses/${id}`} className="card-hover" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 16px', textDecoration: 'none', display: 'flex', gap: 12, alignItems: 'center' }}>
                    <div style={{ width: 28, height: 28, borderRadius: 5, background: 'rgba(157,140,255,0.1)', border: '1px solid rgba(157,140,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'var(--accent)', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
                      P{c.part}
                    </div>
                    <div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 2 }}>{c.partLabel}</div>
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '28px 16px 80px' }}>
        ...
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 'clamp(18px, 4vw, 28px)', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {q.options.map((opt, i) => {
              ...
              if (selected !== null) {
                if (i === q.correctIndex) { bg = 'rgba(45,206,137,0.1)'; borderColor = 'rgba(45,206,137,0.5)'; color = 'var(--green)'; }
                else if (i === selected && i !== q.correctIndex) { bg = 'rgba(247,95,106,0.08)'; borderColor = 'rgba(247,95,106,0.45)'; color = 'var(--red)'; }
              }
              return (
        ...
        {showExp && (
          <div style={{ background: 'rgba(157,140,255,0.06)', border: '1px solid rgba(157,140,255,0.18)', borderRadius: 10, padding: '16px 18px', marginBottom: 16 }}>
